...
public:
	C_Mot(double thresh = 1.0):thresh_validation(thresh),error_sum(0),M_sum(0),FP_sum(0),IS_sum(0),match_count(0),gt_count(0),frame_count(0),tra_count(0)/*,sta_writer(STA_FILE,ios::out)*/{}
	~C_Mot()
	{
		//sta_writer.close();
	}
	...
	inline MOTResult getMOT(bool Det=false)
	{
		...
		rt.ID_switch=IS_sum;
		cout << "MODA: " << (rt.MOTA * 100.0) << " %" << endl;
		cout << "MODP: " << (rt.MOTP * 100.0) << " %" << endl;
		
		if (!Det)
			cout<<"IS: "<<rt.ID_switch<<endl;
		else
		{
			float precision = match_count/(match_count+FP_sum);
			
			cout << "Precision: " << (std::isnan(precision) ? 0.0 : (precision * 100.0)) << " % " << endl;
			cout << "Recall: " << ((match_count / gt_count) * 100.0) << " % " << endl;
		}
		
		cout << "Missing targets: " << ((rt.missing / gt_count) * 100.0) << " % " << endl;
		cout << "False positives: " << ((rt.false_positive / gt_count) * 100.0) << " % " << endl;
		cout << "GT count: " << gt_count << endl;
		cout << "match_count+FP_sum: " << (match_count+FP_sum) << endl;
		return rt;
	}
	...
	void paintFrame(Mat& frame, bool isGroundtruth = false)
	{
		...
		for (unsigned int i=0;i<miss_detection.size();i++)
		{
			rectangle(frame,miss_detection[i].window,Scalar(255,0,255),2);
		}
		for (unsigned int i=0;i<false_alarm.size();i++)
		{
	...
	vector<ResultWindow> readNextFrame()
	{
		...
		if (frame!=NULL)
		{
			...
			while (object!=NULL)//object level
			{
				...
				//cerr << "Rect: [" << res.height << "," << res.width << "," << res.x << "," << res.y << "]" << endl;
